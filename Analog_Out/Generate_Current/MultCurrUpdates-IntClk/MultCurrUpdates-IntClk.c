...
const uInt64 sampsPerChan = 4000; // The number of samples to generate for each channel in the task.
...
int main(void)
{
	...
	for(;i<sampsPerChan;i++)
		data[i] = 0.01*(double)i/(double)sampsPerChan;

	/*********************************************/
	// DAQmx Configure Code
	/*********************************************/
	DAQmxErrChk (DAQmxCreateTask("",&taskHandle));
	DAQmxErrChk (DAQmxCreateAOCurrentChan(taskHandle,physicalChannel,"",minVal,maxVal,units,NULL));
